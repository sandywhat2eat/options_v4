"""
Data Manager for fetching and processing INDEX market data ONLY
Index-specific version - NO STOCKS
"""
...
class DataManager:
    """Handles INDEX-ONLY data fetching and processing operations"""
    
    # Hardcoded index mappings with security IDs
    INDEX_MAPPINGS = {
    ...
    def get_index_symbols(self) -> List[str]:
        """Get hardcoded index symbols that have options data"""
        try:
            # Use hardcoded index list
            index_symbols = list(self.INDEX_MAPPINGS.keys())
            
            # Verify each index has recent options data
            available_indexes = []
            for symbol in index_symbols:
                # Check if options data exists for this index
                ...
                if response.data:
                    available_indexes.append(symbol)
                    logger.info(f"Index {symbol} has options data available")
                else:
                    logger.warning(f"Index {symbol} has no options data")
            
            logger.info(f"Found {len(available_indexes)} indexes with options data")
    ...
    def get_portfolio_symbols(self) -> List[str]:
        """Fetch INDEXES ONLY from option_chain_data table"""
        try:
            # Get only index symbols - NO STOCKS
            index_symbols = self.get_index_symbols()
            logger.info(f"Index-only portfolio: {len(index_symbols)} indexes")
