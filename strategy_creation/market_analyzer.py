...
class MarketAnalyzer:
    ...
    def analyze_market_direction(self, symbol: str, options_df: pd.DataFrame, 
        ...
        try:
            ...
            # If we have actual market view data, use it
            if 'market_view' in locals() and market_view:
                # Extract direction from market_view JSON
                try:
                    # Check for various directional signals in the data
                    bullish_signals = 0
                    bearish_signals = 0
                    
                    # Technical signals
                    if technical_details.get('trend') == 'Uptrend':
                        bullish_signals += 1
                    elif technical_details.get('trend') == 'Downtrend':
                        bearish_signals += 1
                        
                    if technical_details.get('macd_signal') == 'Bullish':
                        bullish_signals += 1
                    ...
                    if options_details.get('smart_money_direction') == 'Bullish':
                        bullish_signals += 1
                    elif options_details.get('smart_money_direction') == 'Bearish':
                        bearish_signals += 1
                    
                    # Price action signals
                    if price_action_details.get('options_momentum') == 'Bullish':
                        bullish_signals += 1
                    elif price_action_details.get('options_momentum') == 'Bearish':
                        bearish_signals += 1
                        
                    # OI distribution signals
                    if options_details.get('oi_distribution', {}).get('directional_bias', 0) > 0.2:
                        bullish_signals += 1
                    elif options_details.get('oi_distribution', {}).get('directional_bias', 0) < -0.2:
                        bearish_signals += 1
                    
                    # Override final score if we have clear signals
                    if bearish_signals > bullish_signals and bearish_signals >= 1:
                        # Force bearish score (more aggressive)
                        final_score = -0.3 - (0.15 * bearish_signals)
                        logger.info(f"Overriding to BEARISH: {bearish_signals} bearish signals vs {bullish_signals} bullish")
                    elif bullish_signals > bearish_signals and bullish_signals >= 1:
                        # Force bullish score  
                        final_score = 0.3 + (0.15 * bullish_signals)
                        logger.info(f"Overriding to BULLISH: {bullish_signals} bullish signals vs {bearish_signals} bearish")
                    elif bearish_signals == bullish_signals and bearish_signals > 0:
                        # Conflicting signals - use weighted calculation but ensure not exactly 0
                        final_score = (
                            technical_score * weights['technical'] +
                            options_score * weights['options'] +
                            price_action_score * weights['price_action']
                        )
                        # Slight bias based on technical trend
                        if technical_details.get('trend') == 'Downtrend':
                            final_score -= 0.1
                        elif technical_details.get('trend') == 'Uptrend':
                            final_score += 0.1
                    else:
    ...
    def _analyze_technical_indicators_enhanced(self, symbol: str, spot_price: float) -> Tuple[float, Dict]:
        ...
        try:
            ...
            if self.technical_analyzer is None:
                # Fallback if technical analyzer not available
                return self._fallback_technical_analysis(symbol, spot_price)
    ...
    def _fallback_technical_analysis(self, symbol: str, spot_price: float) -> Tuple[float, Dict]:
        """Fallback technical analysis when TechnicalAnalyzer not available"""
        try:
            # For testing: return neutral technical score to let options flow dominate
            technical_score = 0.0
            ...
            logger.warning(f"Using fallback technical analysis for {symbol} - returning neutral")
            return technical_score, details
            
        except Exception as e:
            logger.error(f"Error in fallback technical analysis: {e}")
            return 0.0, {}
