...
class ProductionAllocatorRunner:
    ...
    def load_strategies_from_database(self) -> Optional[Dict]:
        ...
        try:
            ...
            ).gte('generated_on', two_days_ago).gte('probability_of_profit', 0.3).gte('total_score', 0.4).order('total_score.desc').execute()
            ...
            for strategy in strategies:
                ...
                if strategy_details_result.data:
                ...
                if hasattr(self.allocator, 'allocated_strategies'):
                    for strategy in self.allocator.allocated_strategies:
                        allocations_list.append({
                            'strategy_id': getattr(strategy, 'strategy_id', strategy.get('id')),
                            'stock_name': getattr(strategy, 'stock_name', strategy.get('stock_name')),
                            'strategy_name': getattr(strategy, 'strategy_name', strategy.get('strategy_name')),
                            'allocation_percent': getattr(strategy, 'allocation_percent', strategy.get('allocation_percent')),
                            'quantum_score': getattr(strategy, 'quantum_score', strategy.get('quantum_score')),
                            'vix_fit_score': getattr(strategy, 'vix_fit_score', strategy.get('vix_fit_score', 0))
                        })
