...
class SophisticatedPortfolioAllocator:
    ...
    def __init__(self, config_path: str = None, db_integration = None):
        ...
        self.external_strategies_data = None  # For passing real strategies from runner
        
        logger.info("Sophisticated Portfolio Allocator initialized")
    
    def set_external_strategies(self, strategies_data: List[Dict]):
        """Set external strategies data from database"""
        self.external_strategies_data = strategies_data
        logger.info(f"Set {len(strategies_data)} external strategies")
    ...
    def analyze_market_environment(self) -> Dict:
        ...
        try:
            # Check if VIX values have already been set by the runner
            if self.current_vix is not None and self.current_vix_percentile is not None:
                vix_data = {
    ...
    def _query_strategies_from_database(self) -> pd.DataFrame:
        ...
        # Use external strategies if provided by runner
        if self.external_strategies_data:
            logger.info(f"Using {len(self.external_strategies_data)} external strategies")
            return pd.DataFrame(self.external_strategies_data)
        
        # Otherwise use mock data (for testing)
        logger.info("No external strategies provided, using mock data")
